object session {import scala.runtime.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(60); 
  ...
  def abs(x: Double) = if (x < 0) -x else x;System.out.println("""abs: (x: Double)Double""");$skip(23); 
  
  
  println("abc");$skip(8); val res$1 = 
  1 + 1;System.out.println("""res1: Int(2) = """ + $show(res$1));$skip(315); 
  def sqrt(x: Double) = {
  ...
  sqrt(4);System.out.println("""res3: Double = """ + $show(res$3));$skip(14); val res$4 = 
  sqrt(1e-20);System.out.println("""res4: Double = """ + $show(res$4));$skip(14); val res$5 = 

...
abstract class TweetSet {
  ...
  def union(that: TweetSet): TweetSet = if(!that.isEmpty) this.incl(that.head).union(that.tail) else this

  // Hint: the method "remove" on TweetSet will be very useful.
  def ascendingByRetweet: Trending = {
    ascendingByRetweet0(new EmptyTrending)
  }
  def ascendingByRetweet0(curr: Trending): Trending = {
      if (isEmpty) curr else remove(findMin).ascendingByRetweet0(curr + findMin)      
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    if (p(elem)) right.filter0(p, left.filter0(p, accu.incl(elem))) else right.filter0(p, left.filter0(p, accu))
  }
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = {
    allTweets.filter(tweet => google.exists(tweet.text.contains))
  }

  val appleTweets: TweetSet = {
    allTweets.filter(tweet => apple.exists(tweet.text.contains))
  }
